...
\stepcounter{rudin}
\begin{theorem}{}{3.3}
    Suppose $\set{s_n}, \set{t_n}$ are complex sequences that converge, with $\linf{n} s_n \rightarrow s$ and $\linf{n} t_n \rightarrow t$. Then:
    \begin{enumerate}
        \item $\linf{n}(s_n + t_n) = s + t$.
        \item $\linf{n} cs_n = cs$ and $\linf{n} (c + s_n) = c + s_n$ for all $c \in \CC$.
        \item $\linf{n}
    \end{enumerate}
\end{theorem}
