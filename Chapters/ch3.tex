...
\begin{definition}{Cauchy Sequences}{3.8}
    A sequence $\set{p_n} \subset X$ is a Cauchy sequence if for all $\e > 0$, there exists $N \in \NN$ such that for all $n, m \geq N$, $d(p_n, p_m) < \e$. 
\end{definition}
...
\begin{definition}{Diameter}{3.9}
    Let $E \subset X$. Then the diameter of $E$, denoted $\diam E$ is defined as $\diam E = \sup\set{d(p, q): p, q \in E}$. 
\end{definition}

\begin{nexample}{}{}
    \begin{enumerate}
        \item If $E = (a, b) \subset \RR$ or $E = [a, b] \subset \RR$, then $\diam E = b - a$.
        \item Let $\set{p_n}$ be a sequence, and define $E_n = \set{p_n}_{n=N}^\infty$ (the tail of the sequence). Then, $\set{p_n}$ is Cauchy if and only if $\lim_{N \rightarrow \infty} \diam E_n = 0$.
    \end{enumerate}
\end{nexample}
