...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=1.5]
        \draw[-latex] (0, 0) -- (2, 0);
        \draw[-latex] (0, 0) -- (0, 2);
        ‎\draw[<->, blue, thick, smooth, samples = 100, domain=0:2, variable = \x] plot(\x, {exp(-1*(\x-1)*(\x-1))});
    \end{tikzpicture}
    
    \caption{Visualization of how $P(X_n)$ concentrates around $p$ as $n \rightarrow \infty$.}
    \label{fig47}
\end{figure}
