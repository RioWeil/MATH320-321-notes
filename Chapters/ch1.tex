...
\begin{ntheorem}{}
    If $E \subset S$ has a maximum element $\alpha$ (that is, an element such that $x \le \alpha$ for all $x \in E$) then $\alpha = \sup(E)$. Similarly, if $E$ has a minimum element $\alpha$, then $\alpha = \inf(E)$.
\end{ntheorem}
