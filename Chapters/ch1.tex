...
Let $z = a + bi \in \CC$. Then, $\Re(z) = a$ is the real part of $z$ and $\Im(z) = b$ is the imaginary part of $z$. The complex conjugate of $z$, denoted by $\bar{z}$, is defined as $\bar{z} = a - bi$. 
\end{definition}
...
\begin{definition}{Absolute Value}{1.32}
    We define the absolute value $\abs{z}$ of a complex number $z$ as $\abs{z} = \sqrt{z\bar{z}}$. Note that if $a \in \RR$ and $z = (a, 0)$, then
    \begin{align*}
        \abs{z} = \sqrt{a^2} =
        \begin{cases}
            a & \text{if $a \geq 0$}
...
\begin{definition}{Absolute Value Properties}{1.33}
    Let $z, w \in \CC$.
    \begin{enumerate}
